...
if [ ! -d ~/pathfinder ]; then
    ...
    git clone https://github.com/GITHUB_REPO.git pathfinder
...
if [ ! -d models/paraphrase-multilingual-MiniLM-L12-v2 ]; then
    ...
    echo -e "${YELLOW}  🤖 Downloading ML model (480MB)...${NC}"
...
if [ ! -f .env ]; then
    ...
    echo -e "${YELLOW}  ⚠️  IMPORTANT: Edit ~/pathfinder/.env and add your API key!${NC}"
...
if systemctl is-active --quiet pathfinder; then
    ...
    echo -e "${YELLOW}  ℹ️  To run manually:${NC}"

    echo -e "     cd ~/pathfinder && source venv/bin/activate"

    echo -e "     uvicorn src.backend.app:app --host 0.0.0.0 --port 8000"
